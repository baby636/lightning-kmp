...
class WaitForFundingConfirmedTestsCommon : LightningTestSuite() {
    ...
    fun `receive FundingLocked`() {
        val (alice, bob) = init(ChannelType.AnchorOutputs, TestConstants.fundingAmount, TestConstants.pushMsat)
    ...
    @Test
    fun `recv Error`() {
        val (_, bob) = init(ChannelType.AnchorOutputs, TestConstants.fundingAmount, TestConstants.pushMsat)
    ...
    fun `recv CMD_FORCECLOSE (nothing at stake)`() {
        val (alice, bob) = init(ChannelType.AnchorOutputs, TestConstants.fundingAmount, 0.msat)
    ...
    fun `on reconnection a zero-conf channel needs to register a zero-confirmation watch on funding tx`() {
        ...
        val (alice, bob) = init(ChannelType.AnchorOutputs, TestConstants.fundingAmount, TestConstants.pushMsat, bobFeatures = bobFeatures)
        val (bob1, _) = bob.processEx(ChannelEvent.Disconnected)
        assertTrue(bob1 is Offline)
