...
class OpenTlvTestsCommon : LightningTestSuite() {
    ...
    fun `channel type TLV`() {
        val testCases = listOf(
            Pair(ChannelType.Standard, Hex.decode("0100")),
            Pair(ChannelType.StaticRemoteKey, Hex.decode("01021000")),
            Pair(ChannelType.AnchorOutputs, Hex.decode("0103101000"))
        )
        ...
        testCases.forEach {
            ...
            val channelType = decoded.get<ChannelTlv.ChannelTypeTlv>()?.let { ct -> ChannelType.fromFeatures(ct.features) }
    ...
    fun `invalid channel type TLV`() {
        ...
        testCases.forEach {
            ...
            assertNotNull(channelType)
            assertNull(ChannelType.fromFeatures(channelType.features))
        }
    }
    ...
    fun `channel version TLV (legacy)`() {
        val testCases = listOf(
            Pair(ChannelType.AnchorOutputs, Hex.decode("fe47000001 04 00000007")),
            Pair(ChannelType.AnchorOutputs, Hex.decode("fe47000001 04 0000000f"))
        )
