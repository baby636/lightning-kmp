...
class WaitForAcceptChannelTestsCommon : LightningTestSuite() {
    ...
    fun `recv AcceptChannel (zero conf)`() {
        ...
        assertEquals(alice1.channelFeatures, ChannelFeatures(setOf(Feature.StaticRemoteKey, Feature.AnchorOutputs, Feature.Wumbo, Feature.ZeroConfChannels)))
    }
    ...
    fun `recv AcceptChannel (missing channel type)`() {
        ...
        assertEquals(error, Error(accept.temporaryChannelId, MissingChannelType(accept.temporaryChannelId, ChannelType.AnchorOutputs.toFeatures()).message))
    }

    @Test
    fun `recv AcceptChannel (invalid channel type)`() {
        val (alice, _, accept) = init()
        val (alice1, actions1) = alice.process(ChannelEvent.MessageReceived(accept.copy(tlvStream = TlvStream(listOf(ChannelTlv.ChannelTypeTlv(ChannelType.Standard))))))
        assertTrue(alice1 is Aborted)
        val error = actions1.hasOutgoingMessage<Error>()
        assertEquals(error, Error(accept.temporaryChannelId, InvalidChannelType(accept.temporaryChannelId, ChannelType.Standard.toFeatures()).message))
    }
    ...
    companion object {
        fun init(
            ...
            assertEquals(accept.tlvStream.get(), ChannelTlv.ChannelTypeTlv(ChannelType.AnchorOutputs))
