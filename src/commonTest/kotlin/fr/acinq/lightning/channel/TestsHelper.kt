...
object TestsHelper {
    ...
    fun init(
        channelType: ChannelType = ChannelType.AnchorOutputs,
    ...
    fun localClose(s: ChannelState): Pair<Closing, LocalCommitPublished> {
        ...
        assertEquals(ChannelType.AnchorOutputs, s.commitments.channelFeatures.channelType)
    ...
    private fun checkSerialization(state: ChannelStateWithCommitments, saveFiles: Boolean = false) {
        ...
        fun maskChannelFeatures(state: ChannelStateWithCommitments): ChannelStateWithCommitments = when (state) {
            is WaitForRemotePublishFutureCommitment -> state.copy(commitments = state.commitments.copy(channelFeatures = ChannelFeatures(ChannelType.AnchorOutputs.features)))
            is WaitForFundingConfirmed -> state.copy(commitments = state.commitments.copy(channelFeatures = ChannelFeatures(ChannelType.AnchorOutputs.features)))
            is WaitForFundingLocked -> state.copy(commitments = state.commitments.copy(channelFeatures = ChannelFeatures(ChannelType.AnchorOutputs.features)))
            is Normal -> state.copy(commitments = state.commitments.copy(channelFeatures = ChannelFeatures(ChannelType.AnchorOutputs.features)))
            is ShuttingDown -> state.copy(commitments = state.commitments.copy(channelFeatures = ChannelFeatures(ChannelType.AnchorOutputs.features)))
            is Negotiating -> state.copy(commitments = state.commitments.copy(channelFeatures = ChannelFeatures(ChannelType.AnchorOutputs.features)))
            is Closing -> state.copy(commitments = state.commitments.copy(channelFeatures = ChannelFeatures(ChannelType.AnchorOutputs.features)))
            is Closed -> state.copy(state = state.state.copy(commitments = state.commitments.copy(channelFeatures = ChannelFeatures(ChannelType.AnchorOutputs.features))))
            is ErrorInformationLeak -> state.copy(commitments = state.commitments.copy(channelFeatures = ChannelFeatures(ChannelType.AnchorOutputs.features)))
        }
