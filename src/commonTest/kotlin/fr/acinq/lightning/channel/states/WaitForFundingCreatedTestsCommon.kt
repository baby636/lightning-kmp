...
class WaitForFundingCreatedTestsCommon : LightningTestSuite() {
    ...
    fun `recv FundingCreated`() {
        val (_, bob, fundingCreated) = init(ChannelType.AnchorOutputs, TestConstants.fundingAmount, TestConstants.pushMsat)
    ...
    fun `recv FundingCreated (with channel origin)`() {
        val (_, bob, fundingCreated) = init(ChannelType.AnchorOutputs, TestConstants.fundingAmount, TestConstants.pushMsat, channelOrigin = ChannelOrigin.PayToOpenOrigin(ByteVector32.One, 42.sat))
        val (bob1, actions1) = bob.process(ChannelEvent.MessageReceived(fundingCreated))
        assertTrue { bob1 is WaitForFundingConfirmed }
        actions1.findOutgoingMessage<FundingSigned>()
        actions1.hasWatch<WatchSpent>()
    ...
    fun `recv FundingCreated (funder can't pay fees)`() {
        val (_, bob, fundingCreated) = init(ChannelType.AnchorOutputs, 1_000_100.sat, 1_000_000.sat.toMilliSatoshi())
        val (bob1, actions1) = bob.process(ChannelEvent.MessageReceived(fundingCreated))
    ...
    @Test
    fun `recv Error`() {
        val (_, bob, _) = init(ChannelType.AnchorOutputs, TestConstants.fundingAmount, TestConstants.pushMsat)
