                    ...
                    is Either.Right -> {
                        ...
                        when {
                            ...
                            lastLocalClosingSigned?.tlvStream?.get<ClosingSignedTlv.FeeRange>()?.let { it.min <= remoteClosingFee && remoteClosingFee <= it.max } == true -> {
                                logger.info { "c:$channelId they chose a fee in our range, publishing closing tx: closingTxId=${signedClosingTx.tx.txid}" }
                                completeMutualClose(signedClosingTx, closingSignedRemoteFees)
                            }
                                ...
                                when {
                                    theirFeeRange != null && !commitments.localParams.isFunder -> {
                                        ...
                                        if (closingFee == remoteClosingFee) {
                                            logger.info { "c:$channelId accepting their closing fees=$remoteClosingFee, publishing closing tx: closingTxId=${signedClosingTx.tx.txid}" }
                                            completeMutualClose(signedClosingTx, closingSignedRemoteFees)
                                        } else {
                                            ...
                                            logger.info { "c:$channelId proposing closing fees=${closingSigned.feeSatoshis}" }
